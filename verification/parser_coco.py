...
class Cycle:
    ...
    def parse(self, cycle_block: list[str]):
        ...
        cycle_pc_pattern = re.compile(r'Cycle\s(\d+):\sPC=(\d+)')
        ...
        for line in cycle_block:
            ...
            match = instruction1_pattern.search(line)
            if match is not None:
                self.instruction1 = match.group(1)
            
            match = instruction2_pattern.search(line)
            if match is not None:
                self.instruction2 = match.group(1)

            match = rf_pattern.search(line)
            if match is not None:
                self.rf = match.group(1)
                
            match = dm_pattern.search(line)
            if match is not None:
...
def main():
    lib = Library(path_str='coco_out.txt')
